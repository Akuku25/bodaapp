...
class SuggestionsAdapter extends ResourceCursorAdapter implements OnClickListener {
    ...
    private static final class ChildViewCache {
        ...
        public ChildViewCache(View v) {
            ...
            this.mIconRefine = (ImageView) v.findViewById(C0055R.id.edit_query);
        }
    }
    ...
    public SuggestionsAdapter(Context context, SearchView searchView, SearchableInfo searchable, WeakHashMap<String, ConstantState> outsideDrawablesCache) {
        super(context, C0055R.layout.abs__search_dropdown_item_icons_2line, null, true);
    ...
    public Cursor runQueryOnBackgroundThread(CharSequence constraint) {
        ...
        try {
            Cursor cursor = getSuggestions(query, 50);
            if (cursor == null) {
                return null;
            }
            cursor.getCount();
    ...
    public Cursor getSuggestions(String query, int limit) {
        ...
        String authority = this.mSearchable.getSuggestAuthority();
        if (authority == null) {
            return null;
        }
        Builder uriBuilder = new Builder().scheme("content").authority(authority).query("").fragment("");
        String contentPath = this.mSearchable.getSuggestPath();
        if (contentPath != null) {
            uriBuilder.appendEncodedPath(contentPath);
        }
        uriBuilder.appendPath("search_suggest_query");
        String selection = this.mSearchable.getSuggestSelection();
        String[] selArgs = null;
        if (selection != null) {
    ...
    private CharSequence formatUrl(CharSequence url) {
        if (this.mUrlColor == null) {
            ...
            this.mContext.getTheme().resolveAttribute(C0055R.attr.textColorSearchUrl, colorValue, true);
