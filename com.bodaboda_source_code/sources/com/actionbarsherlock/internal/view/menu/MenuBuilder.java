...
public class MenuBuilder implements Menu {
    ...
    public void restoreActionViewStates(Bundle states) {
        if (states != null) {
            ...
            if (VERSION.SDK_INT >= 11 || viewStates != null) {
                int itemCount = size();
                for (int i = 0; i < itemCount; i++) {
                    MenuItem item = getItem(i);
                    View v = item.getActionView();
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!(shortcutsVisible && this.mResources.getConfiguration().keyboard != 1 && this.mResources.getBoolean(C0055R.bool.abs__config_showMenuShortcutsWhenKeyboardPresent))) {
            z = false;
        }
